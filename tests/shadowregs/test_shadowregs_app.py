...
def test_command_line_interface_main():
    ...
    assert 'getreg      The command prints the current value of one shadow register.' in result.output
    ...
    assert 'printregs   Print all Shadow registers including theirs current values.' in result.output
...
def test_command_line_interface_loadconfig():
    ...
    assert '-f, --filename TEXT  The name of file used to load a new configuration.' in result.output
    assert '-r, --raw            In loaded configuration will accepted also the computed' in result.output
    assert '--help' in result.output
...
def test_command_line_interface_saveconfig():
    ...
    assert '-r, --raw            The stored configuration will include also the computed' in result.output
    assert '--help' in result.output
...
def test_command_line_interface_printregs_exe_fail():
    """Test for printregs execution menu options."""
    runner = CliRunner()
    cmd = f'-dev imxrt595 -i virtual -s {DebugProbeVirtual.UNIQUE_SERIAL} printregs'
    result = runner.invoke(main, cmd.split())
...
def test_command_line_interface_printregs_r_exe():
    ...
    runner = CliRunner()
    enable_debug = '-o subs_ap={"12":["Exception",12345678],"33554432":[2,0,2,0],"33554440":[0]}'
    cmd = f'-dev imxrt595 -i virtual -s {DebugProbeVirtual.UNIQUE_SERIAL} {enable_debug} printregs -r'
    result = runner.invoke(main, cmd.split())
