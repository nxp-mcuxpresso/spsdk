...
def test_master_boot_image_type():
    for image_type in MasterBootImageType.tags():
        ...
        # CRC
        assert MasterBootImageType.has_crc(image_type) == ('CRC' in MasterBootImageType.name(image_type))
        ...
        # signed
        assert MasterBootImageType.is_signed(image_type) == ('SIGN' in MasterBootImageType.name(image_type).upper()
        ...
        # encrypted
        assert MasterBootImageType.is_encrypted(image_type) == ('ENCR' in MasterBootImageType.name(image_type).upper())
...
@pytest.mark.parametrize(
    ...
    [
        # 2048
        ('selfsign_privatekey_rsa2048.pem', 'selfsign_2048_v3.der.crt',
         'evkmimxrt685_testfffffff_xip_signed2048_no_tz_mbi.bin'),
        # 3072
        ('private_rsa3072.pem', 'selfsign_3072_v3.der.crt',
         'evkmimxrt685_testfffffff_xip_signed3072_no_tz_mbi.bin'),
        # 4096
        ('private_rsa4096.pem', 'selfsign_4096_v3.der.crt',
...
@pytest.mark.parametrize(
    ...
    [
        ('E39FD7AB61AE6DDDA37158A0FC3008C6D61100A03C7516EA1BE55A39F546BAD5', None,
        ...
        (bytes.fromhex('E39FD7AB61AE6DDDA37158A0FC3008C6D61100A03C7516EA1BE55A39F546BAD5'),
         None, 'evkmimxrt685_testfffffff_ram_signed2048_no_tz_mbi.bin'),
        ('E39FD7AB61AE6DDDA37158A0FC3008C6D61100A03C7516EA1BE55A39F546BAD5', 'key_store_rt6xx.bin',
...
def test_signed_ram_single_certificate_no_tz(data_dir, user_key, key_store_filename, expected_mbi):
    ...
    mbi = MasterBootImage(app=org_data, image_type=MasterBootImageType.SIGNED_RAM_IMAGE, load_addr=0x12345678,
                          trust_zone=TrustZone.disabled(),
                          cert_block=cert_block, priv_key_pem_data=priv_key_pem_data,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (KeySourceType.KEYSTORE, None, '8de432f2283a1cb8bb818d41bf9dfafb',
         'evkmimxrt685_testfffffff_ram_encrypted2048_none_keystore_no_tz_mbi.bin'),
        # key source = OTP
        (KeySourceType.OTP, None, 'fff5a54ee37de8f9606c048d941588df',
         'evkmimxrt685_testfffffff_ram_encrypted2048_otp_no_tz_mbi.bin'),
        # key source = KEYSTORE; key store non-empty
        (KeySourceType.KEYSTORE, 'key_store_rt6xx.bin', '0691d67713375bf6effcfb2c7d83321e',
         'evkmimxrt685_testfffffff_ram_encrypted2048_keystore_no_tz_mbi.bin'),
    ]
)
def test_encrypted_ram_single_certificate_no_tz(data_dir, keysource: KeySourceType, keystore_fn: Optional[str],
                                                ctr_iv: str, expected_mbi: str):
    ...
    mbi = MasterBootImage(app=org_data, image_type=MasterBootImageType.ENCRYPTED_RAM_IMAGE, load_addr=0x12345678,
                          trust_zone=TrustZone.disabled(),
                          cert_block=cert_block, priv_key_pem_data=priv_key_pem_data,
                          hmac_key=user_key, key_store=key_store, ctr_init_vector=ctr_init_vector)
...
@pytest.mark.parametrize(
    ...
    [
        # 3 certificates
        (['selfsign_4096_v3.der.crt', 'selfsign_3072_v3.der.crt', 'selfsign_2048_v3.der.crt'], 2,
         'evkmimxrt685_testfffffff_xip_3_certs_no_tz_mbi.bin'),
        # 4 certificates
        (['selfsign_4096_v3.der.crt', 'selfsign_3072_v3.der.crt',
          'selfsign_2048_v3.der.crt', 'selfsign_3072_v3.der.crt'], 2,
         'evkmimxrt685_testfffffff_xip_4_certs_no_tz_mbi.bin'),
        # 2 certificates (first and last)
        (['selfsign_4096_v3.der.crt', None, None, 'selfsign_2048_v3.der.crt'], 3,
         'evkmimxrt685_testfffffff_xip_2_certs_no_tz_mbi.bin'),
    ]
)
def test_signed_xip_multiple_certificates_no_tz(data_dir, der_certificates, root_index, expected_mbi):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (['ca0_v3.der.crt'], ['crt_v3.der.crt'], 'crt_privatekey_rsa2048.pem',
         'evkmimxrt685_testfffffff_xip_chain_2_no_tz_mbi.bin'),
        # 3 certificates in chain
        (['ca0_v3.der.crt'], ['ch3_crt_v3.der.crt', 'ch3_crt2_v3.der.crt'], 'crt2_privatekey_rsa2048.pem',
         'evkmimxrt685_testfffffff_xip_chain_3_no_tz_mbi.bin'),
    ]
)
def test_signed_xip_certificates_chain_no_tz(data_dir, der_certificates, chain_certificates, priv_key, expected_mbi):
    """Test signed image with multiple certificates, different key length
    :param data_dir: absolute path, where test data are located
...
@pytest.mark.parametrize(
    ...
    [
        ("evkmimxrt595_hello_world_ram.bin", 0x20080000, "evkmimxrt595_hello_world_ram_crc_default_tz_mbi.bin"),
    ]
)
def test_plain_ram_crc_default_tz(data_dir, input_img, load_addr, expected_mbi):
    ...
    mbi = MasterBootImage(app=org_data, load_addr=load_addr, image_type=MasterBootImageType.CRC_RAM_IMAGE,
                          trust_zone=TrustZone.enabled())
...
@pytest.mark.parametrize(
    ...
    [
        ("lpcxpresso55s69_led_blinky.bin", "lpc55xxA1.json", "lpc55xx", "lpc55_crc_custom_tz_mbi.bin"),
        ("evkmimxrt685_hello_world.bin", "rt6xx_test.json", "rt6xx",
         "evkmimxrt685_hello_world_xip_crc_custom_tz_mbi.bin"),
        ("evkmimxrt595_hello_world.bin", "rt5xxA0.json", "rt5xx",
         "evkmimxrt595_hello_world_xip_crc_custom_tz_mbi.bin"),
        ("evkmimxrt595_hello_world.bin", "rt5xx_empty.json", "rt5xx",
         "evkmimxrt595_hello_world_xip_crc_custom_tz_mbi.bin"),
        ("evkmimxrt595_hello_world.bin", "rt5xx_few.json", "rt5xx",
         "evkmimxrt595_hello_world_xip_crc_custom_tz_mbi.bin"),
    ]
)
...
def test_base_info(data_dir):
    ...
    output = mbi.info()
    repr_strings = ["Master Boot Image", "Image type", "Image length", "TrustZone", 'HW user mode keys']
    for req_string in repr_strings:
        assert req_string in output, f'string {req_string} is not in the output: {output}'
    # CRC image
    mbi = MasterBootImage(app=bytes(range(64)), image_type=MasterBootImageType.CRC_RAM_IMAGE, load_addr=0x1000)
    output = mbi.info()
    ...
    cert_block = certificate_block(data_dir, ['selfsign_4096_v3.der.crt', 'selfsign_3072_v3.der.crt'], 0)
    mbi = MasterBootImage(app=bytes(range(64)), load_addr=0x12345678, image_type=MasterBootImageType.SIGNED_XIP_IMAGE,
...
def test_multiple_images_with_relocation_table(data_dir):
    ...
    mbi = MasterBootImage(app=img_data, app_table=table, load_addr=0, image_type=MasterBootImageType.CRC_RAM_IMAGE,
