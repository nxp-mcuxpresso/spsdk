...
def open_mboot() -> McuBoot:
    ...
    devs = []
    for _ in range(5):  # try three times to find USB device (wait until shadows registers are ready)
        devs = scan_usb('RT5xx')
        if len(devs) == 1:
            break
    ...
    if len(devs) != 1:
        raise ConnectionError('RT5xx not connected via USB, '
    ...
    if not TEST_IMG_CONTENT:
        write_shadow_regs(data_dir,
                          [
                              ...
                              (0x40130180, 0x00900010),  # BOOT_CFG[0]: DEFAULT_ISP = 1(USB); SECURE_BOOT_EN=1(enabled)
                          ])
...
def test_sb_otfad_keystore(data_dir: str, subdir: str, image_name: str, secure: bool) -> None:
    ...
    otfad.add_key_blob(KeyBlob(0x8001000, 0x80FFFFF, key, counter,
    ...
    otfad.add_key_blob(KeyBlob(0x8FFD000, 0x8FFDFFF, key, counter,
                               zero_fill=bytes(4), crc=bytes(4)))  # zero_fill and crc should be used only for testing !
    otfad.add_key_blob(KeyBlob(0x8FFE000, 0x8FFEFFF, key, counter,
                               zero_fill=bytes(4), crc=bytes(4)))  # zero_fill and crc should be used only for testing !
    otfad.add_key_blob(KeyBlob(0x8FFF000, 0x8FFFFFF, key, counter,
                               zero_fill=bytes(4), crc=bytes(4)))  # zero_fill and crc should be used only for testing !
