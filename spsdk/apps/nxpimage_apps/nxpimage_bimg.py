...
@click.option(
    "-of",
    "--output-format",
    type=click.Choice(["bin", "hex", "srec"], case_sensitive=False),
    default="bin",
    show_default=True,
    help="Output image format",
)
def bootable_image_export_command(config: Config, output: str, output_format: str) -> None:
    ...
    """
    bootable_image_export(config, output, output_format)


def bootable_image_export(config: Config, output: str, output_format: str = "bin") -> None:
    """Export merged boot image blocks into one bootable image."""
    bimg_image = BootableImage.load_from_config(config)
    bimg_image_info = bimg_image.image_info()

    bimg_image_info.save_binary_image(output, file_format=output_format)
