...
def binary_merge_command(config: str, output: str, adjust_offsets: bool) -> None:
    ...
    """
    binary_merge(config, output, adjust_offsets)


def binary_merge(config: str, output: str, adjust_offsets: bool) -> None:
    """Merge binary images together by description in YAML/JSON configuration."""
    ...
    if adjust_offsets:
        min_offset = image.min_offset
        image.update_offsets()
        logger.warning(f"The offsets have been adjusted by {hex(min_offset)} bytes.")
    try:
        image.validate()
    except SPSDKError as exc:
