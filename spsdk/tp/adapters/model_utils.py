...
class ModelConfig(TpIntfDescription):
    ...
    def generate_edh_keys(self) -> Tuple[ec.EllipticCurvePrivateKey, ec.EllipticCurvePublicKey]:
        """Generate Ephemeral Diffie-Hellman key pair."""
        logger.info("Generating Ephemeral Diffie-Hellman keys")
        if self.reuse_edh_keys:
            dh_private = load_private_key(self.get_abspath(self.edh_private_path))
            dh_public = load_public_key(self.get_abspath(self.edh_public_path))
        else:
            dh_private = generate_ecc_private_key(curve_name="secp256r1")

            dh_public = generate_ecc_public_key(dh_private)
            save_ecc_private_key(dh_private, self.get_abspath(self.edh_private_path))
            save_ecc_public_key(dh_public, self.get_abspath(self.edh_public_path))

        assert isinstance(dh_public, ec.EllipticCurvePublicKey)
        assert isinstance(dh_private, ec.EllipticCurvePrivateKey)
        return dh_private, dh_public

    def generate_session_keys(
        self,
        remote_puk_data: bytes,
        edh_private: ec.EllipticCurvePrivateKey,
        ...
        if self.reuse_ses_keys:
            ...
            remote_puk = reconstruct_cryptography_key(remote_puk_data)
            shared_key = edh_private.exchange(ec.ECDH(), remote_puk)
    ...
    def wrap_key(self, wrapping_key: bytes, key_to_wrap: bytes) -> bytes:
        ...
        if not self.do_wrapping:
            return key_to_wrap
        return keywrap.aes_key_wrap(wrapping_key=wrapping_key, key_to_wrap=key_to_wrap)
    ...
    def unwrap_key(self, wrapping_key: bytes, wrapped_key: bytes) -> bytes:
        ...
        if not self.do_wrapping:
            return wrapped_key
        return keywrap.aes_key_unwrap(wrapping_key=wrapping_key, wrapped_key=wrapped_key)
    ...
    def encrypt_data(
        ...
        tag = cmac_calc(data=cipher_text, key=mac_key)
    ...
    def decrypt_data(
        ...
        """
        if not cmac_verify(data=ciphertext, code=tag, key=mac_key):
            return None
    ...
    def encrypt_data_ctr(self, data: bytes, key: bytes, nonce: bytes) -> bytes:
        ...
        if not self.do_encryption:
            ...
            return data
        encryptor = Cipher(
            algorithm=algorithms.AES(key=key), mode=modes.CTR(nonce=nonce)
        ).encryptor()
        ciphertext = encryptor.update(data=data) + encryptor.finalize()
        return ciphertext
...
def ecc_key_to_bytes(key: ec.EllipticCurvePublicKey) -> bytes:
    """Serialize cryptography's ECC public key coordinates into bytes."""
    key_size = key.curve.key_size // 8
    p_x = key.public_numbers().x.to_bytes(length=key_size, byteorder="big")
    p_y = key.public_numbers().y.to_bytes(length=key_size, byteorder="big")
    return p_x + p_y
...
def derive_key(key: bytes, derivation_data: bytes) -> bytes:
    ...
    new_key = cmac_calc(data=derivation_data, key=key)
    return new_key


def cmac_calc(data: bytes, key: bytes) -> bytes:
    ...
    :return: CMAC
    """
    coder = cmac.CMAC(algorithm=algorithms.AES(key=key))
    coder.update(data=data)
    code = coder.finalize()
    return code


def cmac_verify(data: bytes, code: bytes, key: bytes) -> bool:
    """Verify CMAC using AES.

    :param data: Original data
    :param code: CMAC to validate
    :param key: AES key for CMAC computation
    :return: True if CMAC is valid
    """
    checker = cmac.CMAC(algorithm=algorithms.AES(key=key))
    checker.update(data=data)
    try:
        checker.verify(code)
        return True
