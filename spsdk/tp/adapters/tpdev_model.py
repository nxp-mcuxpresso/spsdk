...
class TpDevSwModel(TpDevInterface):
    ...
    def get_challenge(self, timeout: Optional[int] = None) -> bytes:
        ...
        self.challenge = openssl_backend.random_bytes(16)
    ...
    def authenticate_response(self, tp_data: bytes, timeout: Optional[int] = None) -> bytes:
        ...
        is_valid = tp_response.validate(verification_key)  # type: ignore
        if not is_valid:
        ...
        prov_data_container.add_entry(
            DataEntry(
                payload=ecc_key_to_bytes(self.edh_public),
    ...
    def _update_runing_hash(self) -> bytes:
        ...
        new_hash = openssl_backend.hash(data_to_hash)
        return new_hash

    def _sign_log_hash(self, log_hash: bytes) -> bytes:
        log_key = load_private_key(self.config.get_abspath(self.config.data["oem_log_prk_path"]))
        assert isinstance(log_key, ec.EllipticCurvePrivateKey)
        signature = openssl_backend.ecc_sign(log_key, log_hash)
        return signature

    def _get_verification_key(self, nxp_die_id_cert: DataEntry) -> ec.EllipticCurvePublicKey:
        ...
        key = reconstruct_cryptography_key(key_material)
        return key
    ...
    def _generate_oem_certificates(
        ...
        oem_certificates = []
        new_uuid = openssl_backend.random_bytes(16)
        ...
        for oem_die_id_puk in oem_die_id_puks:
            ...
            assert isinstance(issuer_private_key, ec.EllipticCurvePrivateKeyWithSerialization)
            new_signature = issuer_private_key.sign(
                template_data[tbs_slice], ec.ECDSA(hashes.SHA256())
            )
    ...
    def _create_oem_id_template(self) -> None:
        ...
        private_key = load_private_key(self.config.get_abspath(self.config.data["oem_id_prk_path"]))
        ...
        save_crypto_item(cert, cert_path, Encoding.DER)

        logger.debug("Extracting info about the certificate template")
        tbs = cert_raw.find(cert.tbs_certificate_bytes)

        scn = cert.subject.get_attributes_for_oid(NameOID.COMMON_NAME)[0].value
    ...
    def _validate_prod_certificate(self, cert_path: str, puk_path: str) -> None:
        ...
        try:
            ...
            puk.verify(cert.signature, cert.tbs_certificate_bytes, ec.ECDSA(hashes.SHA256()))
            cert_puk = cert.public_key()
            assert isinstance(cert_puk, ec.EllipticCurvePublicKey)
        except InvalidSignature as e:
            raise SPSDKTpError("NXP_PROD certificate signature is invalid") from e
