...
from .model_utils import ModelConfig, ecc_key_to_bytes, get_models_configs
...
class TpTargetSwModel(TpTargetInterface):
    ...
    def __init__(
        ...
        self.oem_id_public_keys: List[Tuple[int, ec.EllipticCurvePublicKey]] = []
        self.edh_private: Optional[ec.EllipticCurvePrivateKey] = None
        self.edh_public: Optional[ec.EllipticCurvePublicKey] = None
    ...
    def prove_genuinity_challenge(self, challenge: bytes, timeout: Optional[int] = None) -> bytes:
        ...
        dh_public_bytes = ecc_key_to_bytes(self.edh_public)
        ...
        for key_index, key in self.oem_id_public_keys:
            key_data = ecc_key_to_bytes(key=key)
            tp_response.add_entry(
                DataEntry(
                    payload=key_data,
    ...
    ) -> List[Tuple[int, ec.EllipticCurvePublicKey]]:
        logger.info("Generating OEM cert public keys")
        oem_id_public_keys: List[Tuple[int, ec.EllipticCurvePublicKey]] = []
    ...
    def _generate_cache_public_key(
        ...
        if use_existing and os.path.exists(public_key_path):
            public_key = load_public_key(self.config.get_abspath(public_key_path))
            assert isinstance(public_key, ec.EllipticCurvePublicKey)
            return public_key

        private_key = generate_ecc_private_key(curve_name="secp256r1")
        public_key = generate_ecc_public_key(private_key)
        save_ecc_private_key(
            private_key,
            self.config.get_abspath(self.config.data[f"oem_id_{key_id}_private"]),
        )
        save_ecc_public_key(
            public_key,
            self.config.get_abspath(self.config.data[f"oem_id_{key_id}_public"]),
        )
        return public_key
    ...
    def _generate_nxp_die_id_devattest_cert(self) -> bytes:
        ...
        if self.config.reuse_die_id_keys and os.path.exists(die_id_auth_prk_path):
            die_id_auth_prk = load_private_key(die_id_auth_prk_path)
        else:
            die_id_auth_prk = generate_ecc_private_key(curve_name="secp256r1")
            save_ecc_private_key(die_id_auth_prk, die_id_auth_prk_path)
        die_id_auth_puk = die_id_auth_prk.public_key()
        assert isinstance(die_id_auth_puk, ec.EllipticCurvePublicKey)

        devattest_cert = Container()
        devattest_cert.add_entry(
            DataEntry(
                payload_type=PayloadType.NXP_DIE_ID_AUTH_PUK,
                payload=ecc_key_to_bytes(die_id_auth_puk),
            )
        )
        ...
        if tp_flags.die_id_cert_version == 1:
            ...
            if self.config.reuse_die_id_keys and os.path.exists(die_id_attest_prk_path):
                die_id_attest_prk = load_private_key(die_id_attest_prk_path)
            else:
                die_id_attest_prk = generate_ecc_private_key(curve_name="secp256r1")
                save_ecc_private_key(die_id_attest_prk, die_id_attest_prk_path)
            die_id_attest_puk = die_id_attest_prk.public_key()
            assert isinstance(die_id_attest_puk, ec.EllipticCurvePublicKey)
            devattest_cert.add_entry(
                DataEntry(
                    payload_type=PayloadType.NXP_DIE_ATTEST_AUTH_PUK,
                    payload=ecc_key_to_bytes(die_id_attest_puk),
                )
            )
        ...
        devattest_cert.add_auth_entry(
            auth_type=AuthenticationType.ECDSA_256,
            key=load_private_key(  # type: ignore
