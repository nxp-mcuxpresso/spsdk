    ...
    def _get_registers(self) -> None:
        ...
        for row in range(start, self.worksheet.max_row + 1):
            ...
            if isinstance(self.worksheet[cell].value, str):
                ...
                access = self.worksheet[sr_access_cr[0] + str(row)].value \
                         if isinstance(self.worksheet[sr_access_cr[0] + str(row)].value, str) else ""
                if any(x in access for x in ["rw", "ro", "wo"]):
                    ...
                    if cells is not None:
                        ...
                        cells = cells.split(':')
                        regs_group_max = cell_utils.coordinate_from_string(cells[1])[1] - \
        ...
        try:
            for row in range(3, self.worksheet.max_row + 1):
                ...
                if new_reg_base != reg_base:
                    ...
                    reg_reverse = False
                    reg_access = "RW"
                    reg = RegsRegister(reg_name, reg_offset, reg_width, reg_description, reg_reverse, reg_access)
...
XLS_TYPES = {
    "1": ShadowRegsXlsToXmlType1,
    "2": ShadowRegsXlsToXmlType2
}
