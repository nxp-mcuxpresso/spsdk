...
@click.option(
    ...
    help="Path to the original flashloader elf file",
    required=True,
)
@click.option(
    "-t",
    "--ide-type",
    type=click.Choice(["mcux", "iar", "keil"]),
    help="Type of tool to use for converting elf -> bin",
    required=True,
)
@click.option(
    "-p",
    "--ide-path",
    type=click.Path(exists=True),
    help="Path to IDE install (root) folder",
    required=True,
)
def main(elf_file: click.File, ide_type: str, ide_path: click.Path) -> None:
    ...
    try:
        ...
        combine_files(
            ...
            f"{THIS_DIR}/preamble.bin",
            f"{THIS_DIR}/flashloader.bin",
        )
...
def get_elf_to_bin_command(
    ide_type: str, ide_path: click.Path, elf_file: click.File, bin_file: str
) -> Optional[str]:
    ...
    if ide_type == "mcux":
        ...
        if not os.path.isfile(tool_path):
            click.echo(f"ARM Object Copy tool was not found: {tool_path}", err=True)
        else:
            return f"{tool_path} --output-target binary {elf_file} {bin_file}"
    if ide_type == "keil":
        tool_path = add_exe(f"{ide_path}/ARM/ARMCC/bin/fromelf")
        if not os.path.isfile(tool_path):
            click.echo(f"ARM image conversion utility was not found: {tool_path}", err=True)
        else:
